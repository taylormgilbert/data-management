...
This tutorial relies on the [BIO3018F prac data mentioned previously](https://docs.google.com/spreadsheets/d/1U9yNaJFyd6kb5vlKzHhcgM6sLEzw_xVtQxewpH7k0Ho/edit?usp=sharing). See [here]( https://github.com/PlantEcologi/BIO3018F_2022/blob/main/PracMaster.pdf) if you'd like more details about the data and practical.
...
# Now let's have a look at the names of the worksheets (tabs) within hte spreadsheet
sheet_names("https://docs.google.com/spreadsheets/d/1U9yNaJFyd6kb5vlKzHhcgM6sLEzw_xVtQxewpH7k0Ho/edit?usp=sharing")
```

Sometimes it shows lots of garble about the online authentication, but finally it shows us that there are three worksheets in the spreadsheet.
...
```{r}
edat <- read_sheet("https://docs.google.com/spreadsheets/d/1U9yNaJFyd6kb5vlKzHhcgM6sLEzw_xVtQxewpH7k0Ho/edit?usp=sharing") # read in data
...
Note that the `skip =` and `nmax =` options are somewhat related and tell the function the number of rows to skip before reading anything and the maximum number of rows to read respectively.
...
Ok, most of what I've shown you is about reading in data and hen manipulating it once you have it read in. While this is handy, there are a few more functions you'll need to help get your data _Tidy_.
...
- When you have multiple values in a single column (or cell)
- When you have gaps in your data

Perhaps the most useful function here is `pivot_longer` from the `tidyr` package.
...
Note that I had to leave out the variable `SoilColour`, because it was a different data type `<chr>` as opposed to `<dbl>`. If you don't convert one of the columns to long format, then it just gets repeated.

You can reverse `pivot_longer` with `pivot_wider`...

So how else are tehse data failing at being _Tidy_?
