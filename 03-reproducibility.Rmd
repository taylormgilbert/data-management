...
# Get Baker data.
cridata <- read_xlsx("/home/jasper/GIT/data-management/data/Reproducibility Survey Raw Data.xlsx", sheet = 1)
...
### Fix here with paste() to make reproducible - currently tweaking the titles to make the figure look nice...
...
(Adapted from "_Five selfish reasons to work reproducibly_" [@Markowetz2015], with a few extras added)
...
- It also helps you get assistance and feedback on your analyses (depending on your supervisor). I'm starting to enforce the use of reproducible research tools among my postgraduate students. Previously, when they needed help with R etc it would take me ages to work out what they were trying to do and to replicate the problem. Now it takes me seconds to just pull their git repository and rerun their code. When it comes to getting feedback and engagement from your supervisors, it helps to give a little too!
...
9. _**Most skills and software used in Reproducible Research are very useful beyond Reproducible Research alone!**_ 
  - e.g. Version control with GitHub or similar - in addition to versioning code etc is great for code management, collaborative projects and can be used for all kinds of things like building websites (e.g. these course notes).
  - It also makes it much easier to start working with **Big Data**... The philosophy with Big Data has shifted from a paradigm of moving the data to the analysis (i.e. putting the data on your local computer and analyzing it there) to moving the analysis to the data (i.e. uploading your analysis code to a server or cloud machine that already has access to the data), largely because the data are too big to download or fit on your laptop. a code repository like GitHub and using containers (contained computing environments) like docker or singularity make this much easier.
...
10. _**Reproducible research skills are highly sought after in careers like data science etc...**_
  ...
  - Even withing biology, more and more environmental organizations and NGOs are hiring data scientists or scientists with strong data and quantitative skills. Some examples I know of:
...
For the rest of this section we'll work through these components and some of the tools that help you achieve this [there's a more thorough treatment of this by @Cooper2017]  [here](https://nhm.openrepository.com/handle/10141/622618).
